...
app.post("/api/book", (req, res) => {
  ...
  bookingsDb.get("SELECT * FROM bookings WHERE date = ? AND name = ?", [date, ip], (err, existingBooking) => {
    if (err) return res.status(500).json({ error: "Database error" });

    if (existingBooking) {
      console.log("iii")
      return res.status(403).json({ error: "Only one booking allowed per IP address per day" });
    }
    ...
    bookingsDb.all("SELECT table_number FROM bookings WHERE date = ?", [date], (err, rows) => {
      ...
      const insertPromises = selectedTables.map(table =>
        new Promise((resolve, reject) => {
          bookingsDb.run("INSERT INTO bookings (name, date, people, table_number) VALUES (?, ?, ?, ?)",
            [ip, date, people, table], function (err) {
              if (err) return reject(err);
              resolve(this.lastID);
            });
        })
